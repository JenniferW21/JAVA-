class Main {
  ...
  void init(){
    ...
    Input.writeFile("Encode1.txt", encodedMsg1); //encodedMsg1 is written into encode1.txt
  ...
  String encode(String txt){
    ...
    for(int x=0; x<=txt.length()-1; x++){
      ch = txt.charAt(x); //define ch 
      ascii = (int)ch; //ascii letter is changed into number
      ascii += 1;
