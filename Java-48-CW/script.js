...
async function init(){  
  let link = "https://orange-happiness-wr5r9g9qpj472p-8500.app.github.dev";// Insert your Dev URL from Activity 47. The activity (DB server) must be running in a separate tab.
  let route= "/employees";

  info = await fetch(link+route); //await process + reads the data; (link+route) is the API
  data = await info.json(); //forces info to become a json
