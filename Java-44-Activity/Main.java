...
class Main {
 ...
 public static void main(String[] args)throws IOException{
  ...
  void init() {
    ...
    sql = "SELECT invoices.invoiceDate, invoices.invoiceId, customers.FirstName, customers.LastName FROM invoices INNER JOIN customers ON invoices.CustomerId=customers.CustomerId WHERE invoices.invoiceDate LIKE '2012-04-%'";
    result = db.runSQL(sql, "table-auto");
    print(result);
    ...
    sql = "SELECT customers.CustomerId, invoice_item.invoiceId, tracks.name, invoice_item.UnitPrice, invoice_item.Quantity FROM invoices INNER JOIN customers ON invoices.CustomerId=Customers.CustomerId  INNER JOIN invoice_items ON invoices.invoiceid=invoice_items.invoiceid LIMIT 20";
    result = db.runSQL(sql,"table-auto");
    // Problem 4:
