class Main {
  public static void main(String[] args) {
    (new Main()).init();
  }
  void print(Object o){ System.out.println(o);}
  void printt(Object o){ System.out.print(o);}

  void init(){
    // #1
    int[] num = {5, 10, 15,60,49,37,27,15,26,23};
    print(avgDistribution(num));

    // #2
    String[] student={"Joe","Snoopy","Homer","Beavis","Zen"};
    print("Type in a student name");
    students = Input.ReadString(student);
    print(findStudent(students,"Homer"));
    
  }

/*
Problem 1:
Write a fn avgDistribution() that accepts an array of integers and performs the following steps:
Step 1: Find the average of the numbers using the fn avgArray() below.
Step 2: Compute the sum of the following: the square of the difference of each integer and the average, i.e.,  (x-avg)^2. 
Step 3: Take the square root of the sum and return this result.

Note: To test this function, create an array of numbers in the init funtion.
*/
  double[] avgDistribution(int[] num){
    int sum = 0;
    double average = avgArray(num); //find average through the function avgArray
    for(int x=0;x<num.length-1;x++){  //for loops goes through every value in the array
      sum += Math.pow(nums[x]-avg,2); //+= continously adds up the values and returns 
    }            //(each integer should be sub by the average) then square the whole thing
    return Math.sqrt(sum); //return the squareroot of the sum (standard dev)
  }
  

/*
  Problem 2:
  Write a fn findStudent() that accepts an array of student names and the name of one student you are looking for in the array. Return true if you find that student in the array; otherwise return false.

  Note: To test this function, create an array of student names in the init function.
*/
    boolean[] findStudent(String[]student, String student){ //datatype[] name(variable,variable) String[]student is the array and String Student is the name we will input
      for(int x=0; x<student.length; x++){ //forloop to check each index value of the array
        if(student[x].equals(student)){ //if the student name in the array == the name that we input
          return true;    //then return true
        }
      }
      return false;       //if not, return false
    }
  

  // Fn to return average of array elements
  double avgArray(int[] n){    
    return sumArray(n)/n.length;
  }
  // Fn to return sum of array elements
  int sumArray(int[] n){
    int sum=0;
    for(int x=0; x<=n.length-1; x++){
      sum+=n[x];
    }
    return sum;
  }

}